...
function initDatePicker() {
  ...
  applyBtn.addEventListener('click', () => {
    const s   = new Date(startIn.value + 'T00:00');
    const e   = new Date(endIn.value + 'T00:00');
    if (isNaN(s) || isNaN(e) || s > e) { showToastNotif('Période invalide : vérifiez les dates'); return; }
...
function initNouvelleTransaction() {
  ...
  createBtn.addEventListener('click', () => {
    ...
    const montantInput = document.getElementById('txMontant');
    if (!(parseFloat(montant) > 0)) {
      montantInput.style.borderColor = 'var(--red)';
      montantInput.focus();
      return;
    }
