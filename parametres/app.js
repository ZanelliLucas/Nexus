...
const EDITABLE_FIELDS = '.field-input:not([readonly]), .field-select, .field-textarea';
let savedValues = [];

function snapshotValues() {
  savedValues = [...document.querySelectorAll(EDITABLE_FIELDS)].map(el => el.value);
}
...
function initSaveDiscard() {
  ...
  document.querySelectorAll(EDITABLE_FIELDS).forEach(el => {
  ...
  if (discardBtn) {
    discardBtn.addEventListener('click', () => {
      ...
      document.querySelectorAll(EDITABLE_FIELDS).forEach((el, i) => { el.value = savedValues[i]; });
